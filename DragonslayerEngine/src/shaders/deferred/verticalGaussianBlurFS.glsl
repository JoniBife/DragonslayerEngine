...
void main()
{
    ...
    float texelHeight = (1.0 / textureSize(previousRenderTexture,0)).y;
    
    for (int i = 1; i < gaussianWeigths.length(); ++i) 
    {
        color += texture(previousRenderTexture, fragTextCoords + vec2(0.0, i * texelHeight)).rgb * gaussianWeigths[i];
        color += texture(previousRenderTexture, fragTextCoords - vec2(0.0, i * texelHeight)).rgb * gaussianWeigths[i];
    }
