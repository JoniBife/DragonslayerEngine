...
void WarriorRenderer::Renderer::render(const Camera& camera, const Lights& lights)
{
	...
	while (!renderQueue.isGeometryEmpty()) {
		...
		renderCommand = renderQueue.dequeueGeometry();
...
bool WarriorRenderer::Renderer::enqueueRender(const RenderCommand& renderCommand)
{
	return renderQueue.enqueueRender(renderCommand);
}

bool WarriorRenderer::Renderer::enqueuePostProcessing(const PostProcessingCommand& postProcessingCommand)
{
	return renderQueue.enqueuePostProcessing(postProcessingCommand);
}

PMaterial* WarriorRenderer::Renderer::createMaterial()
{
