...
void WarriorRenderer::FxaaCommand::sendParametersToShader(const Camera& camera, const FrameBuffer& gBuffer, Texture2D& previousRenderTexture)
{
...
void WarriorRenderer::ACESToneMappingCommand::sendParametersToShader(const Camera& camera, const FrameBuffer& gBuffer, Texture2D& previousRenderTexture)
{
	shaderProgram.setUniform("previousRenderTexture", 0);
	previousRenderTexture.bind(GL_TEXTURE0);
}

WarriorRenderer::ReinhardToneMappingCommand::ReinhardToneMappingCommand()
{
	Shader postProcessingVS(GL_VERTEX_SHADER, "../WarriorRenderer/shaders/deferred/postProcessingVS.glsl");
	Shader toneMappingFS(GL_FRAGMENT_SHADER, "../WarriorRenderer/shaders/deferred/reinhardToneMappingFS.glsl");
	shaderProgram = ShaderProgram(postProcessingVS, toneMappingFS);
	postProcessingVS.deleteObject();
	toneMappingFS.deleteObject();
}

bool WarriorRenderer::ReinhardToneMappingCommand::isValid() const
{
	return true;
}

void WarriorRenderer::ReinhardToneMappingCommand::sendParametersToShader(const Camera& camera, const FrameBuffer& gBuffer, Texture2D& previousRenderTexture)
{
	shaderProgram.setUniform("previousRenderTexture", 0);
	previousRenderTexture.bind(GL_TEXTURE0);
}

WarriorRenderer::SSAOCommand::SSAOCommand(unsigned int sampleSize, unsigned int noiseSize) : sampleSize(sampleSize), noiseSize(noiseSize)
{
	Shader postProcessingVS(GL_VERTEX_SHADER, "../WarriorRenderer/shaders/deferred/postProcessingVS.glsl");
...
bool WarriorRenderer::SSAOCommand::isValid() const
{
	return sampleSize > 0u && sampleSize <= 64u;
}

void WarriorRenderer::SSAOCommand::sendParametersToShader(const Camera& camera, const FrameBuffer& gBuffer, Texture2D& previousRenderTexture)
{
	shaderProgram.setUniform("previousRenderTexture", 0);
	previousRenderTexture.bind(GL_TEXTURE0);
